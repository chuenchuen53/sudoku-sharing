...
import type {
  ...
  NakedPairsTripletsQuadsResult as NakedMultipleResult,
  Pincer,
    ...
    for (let i = 0; i < virtualLines.length; i++) {
      ...
      for (const combination of combinations) {
        const cells = emptyCells.filter(
          (x) => x.candidates && SudokuSolver.isCandidateExtendedTheSet(x.candidates, combination)
        );
    ...
    for (let i = 0; i < virtualLines.length; i++) {
      ...
      for (const combination of combinations) {
        const multiple = emptyCells.filter(({ candidates }) => {
          ...
          return combination.some((sudokuElement) => candidates[sudokuElement] && combination.includes(sudokuElement));
        });
