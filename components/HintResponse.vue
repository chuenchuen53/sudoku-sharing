    ...
    <div v-if="fillStrategy">
      ...
      <div v-if="fillInputValueData.length > 0">
        ...
        <div v-for="(x, index) in fillInputValueData" :key="index">
          <div class="chat chat-start">
            <div class="chat-bubble cursor-pointer">
              <button @click="() => onFillDataClick(x.data)" class="btn-link text-left">
                {{ x.description }}
              </button>
            </div>
          </div>
    ...
    <div v-if="eliminationStrategy">
      ...
      <div v-if="eliminationData.length > 0">
        ...
        <div v-for="(x, index) in eliminationData" :key="index" class="chat chat-start">
          <div class="chat-bubble">
            <button @click="() => onEliminationDataClick(x.data)" class="btn-link text-left">{{ x.description }}</button>
          </div>
        </div>
      </div>
      <div v-else class="chat chat-start">
